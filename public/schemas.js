...
DynamicData.controller('SchemaCtrl', ['$scope', '$rootScope', '$mdSidenav', '$http', '$mdDialog', '$resource', '$mdToast', '$animate', '$Models', '$timeout', '$Cache',
    function ($scope, $rootScope, $mdSidenav, $http, $mdDialog, $resource, $mdToast, $animate, $Models, $timeout, $Cache) {
        ...
        $scope.recurseBuildProperties = function (dataObj, schemaObj) {
            ...
            for (var key in dataObj) {
                if (dataObj[key] === null) {
                    ...
                    schemaObj.properties[key].form = schemaBinder.schemaFormDefinitionsSupport[schemaObj.properties[key].type.name][0];
                } else {
        ...
        function fixProperties(newVal) {
            ...
            if (newVal.items) {
                for (var key in newVal.items.properties) {
                    if (newVal.items.properties[key].key === undefined)
                        continue;
                    
                    if (newVal.items.properties[newVal.items.properties[key].key] !== undefined)
                        continue;
                    
                    
                    newVal.items.properties[newVal.items.properties[key].key] = newVal.items.properties[key];
                    delete newVal.items.properties[key];
                }
            }
            ...
            if (newVal.items) {
                for (var key in newVal.items.properties) {
                    fixProperties(newVal.items.properties[key]);
                }
            }
            else {
                for (var key in newVal.properties) {
        ...
        controller: [
            "$scope", "$controller", "$rootScope", "$mdDialog", "$resource", function ($scope, $controller, $rootScope, $mdDialog, $resource) {
                ...
                function DialogController($scope, $mdDialog, $resource, $compile, $timeout, property) {
                    $scope.inputTypes = JSON.parse(JSON.stringify(schemaBinder.schemaFormDefinitionsSupport));
